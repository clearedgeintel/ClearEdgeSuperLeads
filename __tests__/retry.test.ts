...
// Suppress logger output during tests — pino emits to stdout otherwise.
...
describe('withRetry', () => {
  ...
  test('throws after exhausting retries', async () => {
    ...
    await expect(withRetry(fn, { maxRetries: 2, baseDelayMs: 5 })).rejects.toThrow('ETIMEDOUT');
  ...
  test('does not retry non-retryable status codes', async () => {
    ...
    await expect(withRetry(fn, { maxRetries: 3, baseDelayMs: 5 })).rejects.toThrow('Bad Request');
    expect(fn).toHaveBeenCalledTimes(1);
  });
